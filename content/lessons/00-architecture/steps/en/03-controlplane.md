...
> [!NOTE]
> This lab runs **k3s**, which packs the whole control plane into a **single
> process**. You won't see separate `kube-apiserver` or `etcd` pods. The concepts
> are identical; only the packaging differs.

To see what is running right now, try:
