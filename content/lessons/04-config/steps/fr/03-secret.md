...
**2. Lance un Pod qui monte le Secret en volume** sur `/etc/secret` :
...
**3. Attends Running, puis lis le fichier monté :**

```bash
kubectl get pod secret-demo -w       # attends Running, puis Ctrl-C
...
> [!TIP]
> Les Secrets (et ConfigMaps) montés en volume **se mettent à jour progressivement** quand tu modifies l'objet, le kubelet re-synchronise en quelques secondes à une minute. Les variables d'env sont **figées** au démarrage du conteneur et nécessitent un redémarrage du Pod. Exception : un montage `subPath` ne se met **pas** à jour automatiquement, même en volume.
